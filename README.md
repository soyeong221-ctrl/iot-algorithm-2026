...
* 데이터를 기준값에 따라 오름차순(작은 값부터 큰 값으로), 내림차순(큰 값부터 작은 값으로) 데이터 나열
    - 참고사이트 : [Visualgo](https://visualgo.net/en/sorting)

1. 버블정렬 : [소스](./basic/algorithm04/app02/app02.c)
    - 이웃한 두 요소의 대소관계로 비교하여 교환하며 변환하는 정렬(밀어내기)
    - 가장 큰 값을 끝으로 밀어내는 알고리즘
    - 시간복잡도 : 평균 $O(n^2)$ / 최악 $O(n^2)$ / 최선 $O(n)$
 

2. 선택정렬
    - 가장 작은 요소부터 선택해 알맞은 위치로 옮겨서 순서대로 정렬(찍어서 교체하기)
    - 가장 작은 값을 앞으로 보내는 알고리즘
    - 시간복잡도 : 모두 $O(n^2)$

3. 삽입정렬
    - 이미 정렬된 부분에 새 원소를 끼워 넣는 정렬(카드 정리)
    - 시간복잡도 : 평균 $O(n^2)$ / 최악 $O(n^2)$ / 최선 $O(n)$

4. 셸정렬
    - 삽입정렬을 멀리 떨어진 간격으로 먼저 수행한 뒤 점점 간격을 줄여가는 정렬
    - 삽입정렬을 업그레이드해서 멀리 있는 값부터 정리
    - 삽입정렬이 완전 뒤죽박죽일 때 시간복잡도가 좋지 않음
    - 시간복잡도 : 평균 $O(n^{1.3})$ ~ $O(n^{1.5})$ / 최악 $O(n^2)$ / 최선 $O(n log n)$

5. 퀵정렬
    - 기준값을 정하고, 작은 값은 왼쪽, 큰 값은 오른쪽으로 나눠서 재귀적으로 정렬
    - 시간복잡도 : 평균 $O(n log n)$ / 최악 $O(n^2)$ / 최선 $O(n log n)$

6. 병합정렬
    - 반으로 나눠서 각각 정렬하고, 다시 합치는 방식의 정렬
    - 시간복잡도 : 평균 $O(n log n)$ / 최악 $O(n log n)$ / 최선 $O(n log n)$

7. 정렬 평균기준 반복 횟수 (n = 10,000개)
...
1. 부루트-포스 - 모든 문자가 전부 일치하는지 전부 비교하는 방법
    - 비밀번호 알아내기에는 횟수제한 등으로 막힘

2. KMP - 부루트-포스 방법에서 패턴 형태로 변경된 비교 방법

3. 보이어-무어 - 부루트-포스와 KMP 상에서 패턴으로 옮길 크기 정해서 검색하는 방법
...
2. 연결 리스트 - 포인터로 각 `노드`가 연결된 형태
    - 배열과 달리 중간에 데이터가 빠지거나 추가되어도 배열처럼 데이터를 옮기는 처리 필요 없음

3. 현재는 C#, Java, Python 등은 배열사용 거의 안 함. 리스트로 사용.
